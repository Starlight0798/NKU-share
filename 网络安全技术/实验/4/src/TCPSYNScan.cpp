...
bool TCPSYNScan(const std::string& hostIP, int port) {
    int sockfd;
    ...
    if (sendto(sockfd, packet, ntohs(iph->ip_len), 0, (struct sockaddr *)&sa, sizeof(sa)) < 0) {
        std::cerr << "Error sending SYN packet: " << strerror(errno) << std::endl;
        close(sockfd);
        return false;
    }
    ...
    if (recv_len < 0) {
        if (errno == EWOULDBLOCK || errno == EAGAIN) {
            std::cout << "No response, port " << port << " is filtered." << std::endl;
            close(sockfd);
            return false;
        } else {
    ...
    if (recvTcph->th_flags & TH_RST) {
        ...
        close(sockfd);
        return false;
    } else if (recvTcph->th_flags & TH_SYN && recvTcph->th_flags & TH_ACK) {
        std::cout << "Received SYN-ACK, port " << port << " is open." << std::endl;
        close(sockfd);
        return true;
    } else {
        std::cout << "Unexpected response, port " << port << " is filtered." << std::endl;
        close(sockfd);
        return false;
    }
}
