...
// 构造函数
ChatRoomServer::ChatRoomServer(UINT port, UINT client, string key) {
	// 初始化服务器相关常量
	PORT = port;
	MAX_CLIENTS = client;
	KEY = key;
...
DWORD WINAPI ChatRoomServer::ClientHandler(LPVOID pParam) {
	...
	// 解密
	client->username = DES::vecToStr(pThis->des->decrypt(buffer));
	...
	while (true) {
		...
		// 解密
		buffer.resize(bytes);
		string decmsg = DES::vecToStr(pThis->des->decrypt(buffer));
...
void ChatRoomServer::BroadcastMessage(const string& msg) {
	// 先将string转换为vector<uint8_t>加密
	std::vector<uint8_t> encmsg = des->encrypt(DES::strToVec(msg));
	for (UINT i = 0; i < MAX_CLIENTS; i++) {
...
int main() {
	ChatRoomServer server(12720, 50, "133457799BBCDFF1");  // 选择端口和客户数量和DES密钥
	try {
		server.Start();
