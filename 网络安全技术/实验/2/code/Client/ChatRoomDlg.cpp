...
void CChatRoomDlg::OnBnClickedButtonConnect() {
	...
	// 接收服务器RSA公钥
	{
		...
		int ret = recv(SockClient, reinterpret_cast<char*>(buffer.data()), BUFFER_SIZE, 0);
		if (ret <= 0) {
			MessageBox("接收RSA公钥失败！");
			closesocket(SockClient);
			SockClient = INVALID_SOCKET;
			WSACleanup();
			return;
		...
		buffer.resize(ret);
		this->pub_key.n = *reinterpret_cast<uint64_t*>(buffer.data());
		this->pub_key.e = *reinterpret_cast<uint64_t*>(buffer.data() + sizeof(uint64_t));
		PrintMsg("系统", CString(("RSA公钥: (" + std::to_string(pub_key.n) + ", " + std::to_string(pub_key.e) + ")").c_str()));
	}

	// 生成DES密钥并用RSA公钥加密发送
	{
		CString tmp(DES::generateKey().c_str());
		key = tmp;
		PrintMsg("系统", "生成DES密钥: " + key);
	}
	des = new DES(key.GetBuffer());
	GetDlgItem(IDC_DES_KEY)->SetWindowText(key);
	std::vector<uint8_t> encKey = RSA::encrypt(RSA::strToVec(key.GetBuffer()), pub_key);
	send(SockClient, reinterpret_cast<const char*>(encKey.data()), encKey.size(), 0);
	PrintMsg("系统", "DES密钥已发送！");
