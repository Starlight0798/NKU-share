...
void CChatRoomDlg::OnBnClickedButtonExit() {
	if (MessageBox("确定要退出聊天室？", "提示", MB_YESNO | MB_DEFBUTTON2) == IDYES) {
		...
		if (SockClient != INVALID_SOCKET) {
			...
			send(SockClient, exitMsg, strlen(exitMsg), 0);
...
void CChatRoomDlg::OnBnClickedButtonSend() {
	...
	else if(send(SockClient, CT2A(strMsg.GetBuffer()), strMsg.GetLength(), 0) == SOCKET_ERROR){
		MessageBox("发送消息失败！");
		isConnect = false;
	}
...
void CChatRoomDlg::OnBnClickedButtonConnect() {
	...
	// 判断上述信息合法
	if (strIP.IsEmpty() || strPort.IsEmpty() || strName.IsEmpty()){
		MessageBox("请填写完整信息！");
		WSACleanup();
		return;
	}
	...
	// 发送用户名给服务器
    send(SockClient, CT2A(UserName.GetBuffer()), UserName.GetLength(), 0);
...
DWORD WINAPI CChatRoomDlg::ReceiveMessages(LPVOID pParam) {
	...
	while (true) {
		...
		int ret = recv(pThis->SockClient, buffer, sizeof(buffer), 0);
		if (ret <= 0) {
		...
		pThis->PostMessage(WM_UPDATE_CHAT_MSG, 0, (LPARAM)new CString(strMsg));
	}
	closesocket(pThis->SockClient);
	pThis->SockClient = INVALID_SOCKET;
	return 0;
...
void CChatRoomDlg::PrintMsg(const CString& Name, const CString& strMsg) {
	CString outMsg = GetCurrTime() + " " + Name + ": " + strMsg + "\n";
