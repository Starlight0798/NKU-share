...
void UDPClient::PrintsendBuffer() {
	std::lock_guard<std::mutex> lock(mtx);
	std::queue<UDPPacket> tmp = sendBuffer;
	std::string str = "SendBuffer: ";
	while (!tmp.empty()) {
		str += std::to_string(tmp.front().getHeader().seqNum) + " ";
		tmp.pop();
	}
	UDP::Print(str, INFO);
}
...
void UDPClient::sendFileData(const std::string& filePath) {
	...
	while (true) {
		...
		// 超时重传逻辑
		if (GetTickCount64() - timer_start > timeoutMs) {
			// 打印出base和sendbuffer内容
			Print("Timeout, resend Packet in the Window! Current base: " + std::to_string(base), WARN);
			PrintsendBuffer();

			// 从缓冲区中获取所有包(均为未确认)并重传，实际上就是窗口内的包
			std::queue<UDPPacket> tmp = sendBuffer;
			while (!tmp.empty()) {
				UDPPacket packet = tmp.front();
	...
	if (hrecv) CloseHandle(hrecv); // 关闭接收线程
...
DWORD WINAPI UDPClient::receiveAck(LPVOID pParam) {
	...
	static uint16_t lastack = UINT16_MAX;	// 记录上一次收到的ACK，用于甄别重复ACK
	...
	while (client->running) {
		...
		if (recvLen > 0) {
			...
			if (packet.isFlagSet(Flag::ACK)) {
				...
				// 第一次收到ACK包
				if (lastack == UINT16_MAX) {
					lastack = ack;
				}

				// 收到重复的ACK包
				else if (ack <= lastack) {
					client->Print("Recieve duplicate ACK packet with ackNum: " + std::to_string(ack), WARN);
				}
				...
				// 收到新的ACK包
					...
					// 累计确认机制，老的ACK丢了不要紧，最新的ACK发来了就行
					// 即ack有可能等于client->base，也可以大于client->base
					while (ack > client->base) {
						std::lock_guard<std::mutex> lock(client->mtx);
						client->sendBuffer.pop(); // 从缓冲区中删除已确认的包
						client->base++;		      // 更新窗口基序号
					}
				}
			}
