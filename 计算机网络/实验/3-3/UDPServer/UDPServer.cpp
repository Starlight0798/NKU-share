...
// 构造函数
UDPServer::UDPServer(UINT port, uint32_t window_size) {
	this->port = port;
...
bool UDPServer::receivePacket(UDPPacket& packet) {
	// 模拟丢包和延时处理参数
	const int fixedDelay = 50;	// 延时50ms
	const int drop_every = 33;	// 丢包率为3%
	static std::default_random_engine generator_drop, generator_delay;
	static std::uniform_int_distribution<int> delayDistribution(0, drop_every); // 每drop_every个包中随机选择一个进行延时
	static std::uniform_int_distribution<int> lossDistribution(0, drop_every); // 每drop_every个包中随机选择一个进行丢包
